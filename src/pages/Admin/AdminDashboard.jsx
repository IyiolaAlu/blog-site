...
const AdminDashboard = () => {
  ...
  const handleCreate = async () => {
    try {
      const res = await axios.post(
        "https://blog-siteb.onrender.com/api/posts",
        newPost,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://blog-siteb.onrender.com/api/posts/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const submitEdit = async () => {
    try {
      ...
      setPosts(
        posts.map((p) => (p._id === editingPost._id ? res.data.post : p))
      );
      setEditingPost(null);
    } catch (error) {
      console.error(error);
          ...
          <div className={styles["dashboard-content"]}>
              ...
              <div className={styles["form-card"]}>
                ...
                <button
                  className={`${styles["submit-btn"]} ${
                    editingPost ? styles.edit : styles.create
                  }`}
            ...
            <div className={styles["posts-column"]}>
              <div className={styles["posts-header"]}>
                ...
                <span className={styles["posts-count"]}>
                  {posts.length} posts
                </span>
              </div>
